\section{功能需求}
    \subsection{CPU}
        ...
        \subsubsection{乘法器}
            ...
            \item
            由于乘法指令需要的计算时间较长，因此在乘法单元中加入乘法是否完成的标志位，标志位连接到CPU的控制单元中。
            当乘法运算真正完成之后，再将此标志位置1，CPU的控制单元进入下一条指令的执行。
            \item
        ...
        \subsubsection{扩展部分}
            ...
            初步想法为：首先使用仿真工具ModelSim或者Xilinx自带的ISim进行仿真，
