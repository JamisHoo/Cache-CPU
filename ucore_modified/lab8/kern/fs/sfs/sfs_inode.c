...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2011011278 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    uint32_t blkend = endpos % SFS_BLKSIZE;
    void* bufptr = buf;
    bool single = 0;
    if (blkoff > 0) {
        uint32_t blklen = (nblks > 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);
        assert(sfs_bmap_load_nolock(sfs, sin, blkno, &ino) == 0);
        // cprintf("---------------(BLK 0x%08x) 0x%08x LEN: 0x%08x\n", blkno, blkoff, blklen);
        assert(sfs_buf_op(sfs, bufptr, blklen, ino, blkoff) == 0);
        bufptr += blklen;
        blkno++;
        if (nblks > 0)
            nblks--;
    ...
    if (nblks > 0) {
        assert(sfs_bmap_load_nolock(sfs, sin, blkno, &ino) == 0);
        // cprintf("---------------BLK 0x%08x LEN: 0x%08x\n", blkno, nblks);
        assert(sfs_block_op(sfs, bufptr, ino, nblks) == 0);
        bufptr += nblks * SFS_BLKSIZE;
    }
    if (!single && blkend > 0) {
        assert(sfs_bmap_load_nolock(sfs, sin, endpos / SFS_BLKSIZE, &ino) == 0);
        // cprintf("---------------(BLK 0x%08x) 0x%08x LEN: 0x%08x\n", blkno, 0, blkend);
        assert(sfs_buf_op(sfs, bufptr, blkend, ino, 0) == 0);
        bufptr += blkend;
    }
    alen = bufptr - buf;
out:
